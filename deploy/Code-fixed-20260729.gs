...
function login_(payload, requestId) {
  ...
  // PBKDF2 is intentionally outside every Apps Script lock.
 ...
 */
/** Apps Script cryptographic adapters. Stored PIN values are versioned PBKDF2 records. */
const PIN_HASH_PREFIX_ = 'PBKDF2-SHA256$v1$';
const PIN_PBKDF2_ITERATIONS_ = 100000;
...
function derivePbkdf2Sha256_(pin, salt, iterations, length) {
  const key = Array.prototype.slice.call(Utilities.newBlob(String(pin)).getBytes());
  const blocks = Math.ceil(length / PIN_HASH_BYTES_);
  let output = [];
  for (let block = 1; block <= blocks; block += 1) {
    const blockBytes = [0, 0, 0, block];
    let current = Array.prototype.slice.call(Utilities.computeHmacSha256Signature(salt.concat(blockBytes), key));
    const aggregate = current.slice();
    for (let iteration = 1; iteration < iterations; iteration += 1) {
      current = Array.prototype.slice.call(Utilities.computeHmacSha256Signature(current, key));
      for (let byteIndex = 0; byteIndex < aggregate.length; byteIndex += 1) aggregate[byteIndex] ^= current[byteIndex];
    }
    output = output.concat(aggregate);
  }
...
function createPinHash_(pin) {
  ...
  const derived = derivePbkdf2Sha256_(String(pin), salt, PIN_PBKDF2_ITERATIONS_, PIN_HASH_BYTES_);
  return PIN_HASH_PREFIX_ + PIN_PBKDF2_ITERATIONS_ + '$' + Utilities.base64EncodeWebSafe(salt) + '$' + Utilities.base64EncodeWebSafe(derived);
}
...
function verifyPinHash_(pin, storedHash) {
  ...
  try {
    const salt = Array.prototype.slice.call(Utilities.base64DecodeWebSafe(parts[3]));
    const expected = Array.prototype.slice.call(Utilities.base64DecodeWebSafe(parts[4]));
    if (salt.length !== 16 || expected.length !== PIN_HASH_BYTES_) return false;
    if (!isCanonicalWebSafeBase64_(parts[3], salt) || !isCanonicalWebSafeBase64_(parts[4], expected)) return false;
    const actual = derivePbkdf2Sha256_(String(pin), salt, iterations, expected.length);
...
function isCanonicalWebSafeBase64_(encoded, bytes) {
  return Utilities.base64EncodeWebSafe(bytes) === encoded;
}
